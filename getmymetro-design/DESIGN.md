...
- **Superseded (September 2026): the board is one white tile per direction**, Aluva first, inside the soft card — `[←] Aluva … 2 min` over `6:18 PM … then 9, 16 min`: teal arrow badge, name and the next train's clock on the left; the countdown and, under it, the trains after it on the right. (Not "then …" under the name — that read as "Aluva then 6:32" before the next train's time.) One column; two once the card itself is 640px wide (container query). No `LineTrack` on the board (it read as a slider handle), no "N stations", and no lane-alignment spacers — tiles do not line up with each other.
...
- **Product decision (September 2026): the board no longer prints "Ends at Muttom / Kadavanthra".** The short-turn sentence is still read to screen readers with the next train, and the last-train panel still names the short-turn. This departs from CLAUDE.md findings 9–10, which say boards must label the 20 short-turns.
